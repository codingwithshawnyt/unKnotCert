...
Our main result is a separation between virtual and classical unknotting complexity: for the $D_{28}$ and $D_{43}$ diagrams, $\mathrm{Top}_v(D) = \mathrm{cr}(D)$ but $\mathrm{Top}_c(D) = \mathrm{cr}(D) + 3$ (by Burton et al.\ [4]), yielding a gap of $3$ crossings in both cases.
This provides rigorous examples from both known families of hard unknots (the Freedman--He--Wang generalization and the Goeritz generalization) where algebraic flexibility of Gauss words strictly reduces the minimax crossing barrier compared to geometric (planar) Reidemeister moves.
...
\item We prove a \emph{classical--virtual complexity gap} for $D_{28}$ and $D_{43}$: $\mathrm{Top}_v(D_{28}) = 28 < 31 = \mathrm{Top}_c(D_{28})$ and $\mathrm{Top}_v(D_{43}) = 43 < 46 = \mathrm{Top}_c(D_{43})$.
These are rigorous examples from both known families of hard unknots --- the Freedman--He--Wang generalization ($D_{28}$) and the Goeritz generalization ($D_{43}$) --- where virtual unknotting complexity is strictly lower than classical complexity.
...
For the $D_{28}$ and $D_{43}$ diagrams of Burton et al.~\cite{Burton2020Hard},
\[
\mathrm{Top}_v(D_{28}) = 28 < 31 = \mathrm{Top}_c(D_{28}), \qquad
\mathrm{Top}_v(D_{43}) = 43 < 46 = \mathrm{Top}_c(D_{43}).
\]
...
\begin{proof}
The virtual upper bounds $\mathrm{Top}_v(D_{28}) \leq 28$ and $\mathrm{Top}_v(D_{43}) \leq 43$ are established by the monotone-reducing paths in \cref{thm:virtual_barriers}.
The virtual lower bounds follow from the empty searches at $N = 27$ and $N = 42$, respectively.
...
Similarly, Burton et al.\ prove $m_c(D_{43}) \geq 3$, establishing $\mathrm{Top}_c(D_{43}) \geq 46$.
...
In the classical category, the same crossing pairs can only be de-interleaved by first performing R3 and R1-up/R2-up moves that re-route the diagram through a higher crossing number, reaching $\mathrm{cr}(D) + 3$ crossings before reducing.
...
\begin{remark}
The gap of exactly $m_c = 3$ for both $D_{28}$ and $D_{43}$ is consistent with the interpretation that the three extra crossings required classically are precisely the overhead needed to maintain planarity while performing the algebraic simplification.
The fact that this gap appears in diagrams from both the Freedman--He--Wang and Goeritz families suggests it is a structural feature of the constructions, not an artifact of a single diagram.
\end{remark}
...
\begin{table}
\caption{Summary of classical--virtual gaps for all diagrams with $m_c > 0$.}
\label{tab:gap}
\begin{center}
\begin{tabular}{lcccccc}
\toprule
Diagram & $\mathrm{cr}(D)$ & $\mathrm{Top}_v(D)$ & $\mathrm{Top}_c(D)$ & Gap & Family \\
\midrule
...
\caption{Comparison of virtual minimax barriers $\mathrm{Top}_v(D)$ (this paper) with classical extra crossings $m_c(D)$ (Burton et al.~\cite{Burton2020Hard}). For $D_{28}$ and $D_{43}$, the classical barrier exceeds the virtual barrier by $3$ crossings; for Goeritz and Culprit, the gap is $1$.}
...
Our data suggests that many do: $D_{28}$, $D_{43}$ (with $m_c = 3$), and Ochiai~II (with $m_c = 0$) all admit such paths, while Goeritz ($m_c = 1$) and Culprit ($m_c = 1$) do not --- they require R3 moves even in the virtual category.
...
Both $D_{28}$ (Freedman--He--Wang family) and $D_{43}$ (Goeritz generalization) exhibit a gap of $3$.
